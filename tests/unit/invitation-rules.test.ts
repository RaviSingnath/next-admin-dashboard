...
const PAST = dayBeforeDate(); // before NOW — invite is expired
const FUTURE = dayAfterDate(); // after NOW — invite is still active
...
describe("assertInviteIsResendable", () => {
  ...
  describe("does not throw", () => {
    it("when status is pending and the invite has expired", () => {
      const invite = createInvite({ status: "pending", expires_at: PAST });

      expect(() => assertInviteIsResendable(invite, NOW)).not.toThrow();
    });
  });
  ...
  describe("throws when status is not pending", () => {
    it("status is onboarding — invite was already accepted", () => {
      const invite = createInvite({ status: "onboarding", expires_at: PAST });

      expect(() => assertInviteIsResendable(invite, NOW)).toThrow(
        "This invite has already been accepted and is in onboarding",
      );
    });

    it("status is accepted", () => {
      const invite = createInvite({ status: "accepted", expires_at: PAST });
    ...
    it("status is revoked", () => {
      const invite = createInvite({ status: "revoked", expires_at: PAST });
    ...
    it("status is cancelled", () => {
      const invite = createInvite({ status: "cancelled", expires_at: PAST });
  ...
  describe("throws when status is pending but the invite has not expired", () => {
    it("expires_at is in the future", () => {
      const invite = createInvite({ status: "pending", expires_at: FUTURE });
